...
class VmInfo():
    ...
    def check_availability_state(self):
        for volume in self.vm.volumes.values():
            if volume.ephemeral:
                continue
            if volume.pool not in self.vm.app.pools:
                return False
            if volume.source is not None:
                if volume.source not in (
                    pool_volume.vid
                    for pool in self.vm.app.pools.values()
                    for pool_volume in pool.volumes
                ):
                    return False
            elif volume.vid != "":
                if volume.vid not in (
                    pool_volume.vid
                    for pool_volume in self.vm.app.pools[volume.pool].volumes
                ):
                    return False
        return True
