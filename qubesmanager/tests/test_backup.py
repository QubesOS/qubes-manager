...
def test_00_load_backup(backup_dlg):
    ...
    for i in range(backup_dlg.select_vms_widget.available_list.count()):
        ...
        # the text is in the form of vmname (klass) (size)
        vm, klass, _ = item.text().split(" ", maxsplit=2)
        avail_vms.append(vm + " " + klass)
    for i in range(backup_dlg.select_vms_widget.selected_list.count()):
        item = backup_dlg.select_vms_widget.selected_list.item(i)
        # the text is in the form of vmname (klass) (size)
        vm, klass, _ = item.text().split(" ", maxsplit=2)
        selected_vms.append(vm + " " + klass)
...
def _mock_settings(*args, **kwargs):
    if 'w' in args[1]:
